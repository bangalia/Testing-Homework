...
class MainTests(unittest.TestCase):
    ...
    def test_book_detail_logged_out(self):
        ...
        #  Uses helper functions to create books, authors, user
        create_books()
        create_user()
        # Makes a GET request to the URL /book/1, check to see that the
        # status code is 200
    ...
    def test_update_book(self):
        ...
        # Makes sure the book was updated as we'd expect
    ...
    def test_create_book(self):
        ...
        # Makes sure book was updated as we'd expect
    ...
    def test_favorite_book(self):
        # Logins as the user me1
        create_books()
        create_user()
        login(self.app, 'me1', 'password')

        # Makes a POST request to the /favorite/1 route
        post_data = {
            'book_id': 1
        }
        self.app.post('/favorite/1', data=post_data)
        # Verifies that the book with id 1 was added to the user's favorites
        response = self.app.get('/profile/me1', follow_redirects=True)
        self.assertEqual(response.status_code, 200)
    ...
    def test_unfavorite_book(self):
        # Logins as the user me1, and add book with id 1 to me1's favorites
        create_books()
        create_user()
        login(self.app, 'me1', 'password')
        ...
        self.app.post('/favorite/1', data=post_data)
        # Verifies that the book with id 1 was removed from the user's 
        # favorites
        response = self.app.get('/profile/me1', follow_redirects=True)
        self.assertEqual(response.status_code, 200)
